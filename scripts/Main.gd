...
func clear_all() -> void:
    ...
    bullets.clear(); enemies.clear(); enemy_bullets.clear(); items.clear(); effects.clear(); clouds.clear(); stars.clear(); boss.clear(); boss_missile.clear(); _clear_attack_banner(); _clear_radio_scan(); boss_active = false; boss_dead = false; boss_intro = true
...
func _update_game(delta: float) -> void:
    ...
    if extra_flight:
        ...
        if enemies_spawned >= enemies_to_clear and enemies.size() == 0 and not boss_active and not boss_dead:
            _spawn_boss()
        elif boss_dead:
            level_complete()
...
func _spawn_enemy() -> void:
    ...
    var e = _sprite("enemy%d" % type, Vector2(rng.randf_range(36,W-36), -30), game_root)
    e.z_index = 3
    ...
    var drift := rng.randf_range(-30,30) * speed_mul
    enemies_spawned += 1
    var shoot_delay: float = rng.randf_range(0.95, 2.9) / max(0.1, shoot_mul)
    var enemy_weapon := _random_enemy_weapon()
    enemies.append({"node":e, "hp":hp, "speed":speed, "drift":drift, "shoot":shoot_delay, "score":100*hp, "weapon":enemy_weapon})
...
func _update_bullets(delta: float) -> void:
    for arr in [bullets, enemy_bullets]:
        for i in range(arr.size()-1, -1, -1):
            ...
            if n.position.y < -30 or n.position.y > H+30:
                n.queue_free(); arr.remove_at(i)

func _update_enemies(delta: float) -> void:
    for i in range(enemies.size()-1, -1, -1):
        var d = enemies[i]; var n: Sprite2D = d.node
        n.position.y += d.speed * delta; n.position.x += sin(game_time*2.0+n.position.y*0.03) * d.drift * delta
        d.shoot -= delta
        if d.shoot <= 0 and n.position.y > 20 and n.position.y < H-120:
            ...
            eb.z_index = 4
            enemy_bullets.append({"node":eb, "speed":(150.0 + level * 28.0) * (0.9 + shoot_mul * 0.18), "kind":"enemy_weapon", "weapon":eweapon, "damage":edamage})
            d.shoot = rng.randf_range(1.7 - level*0.16, 3.6 - level*0.22) / max(0.1, shoot_mul)
        if n.position.y > H+40:
...
func _update_boss(delta: float) -> void:
    ...
    if b == null or not is_instance_valid(b):
        return
    # El Acorazado debe permanecer siempre completamente visible.
    # AFG-acorazado43.png tiene frames con alpha decreciente; para el port 1:1
    # usamos siempre el primer frame visible y no animamos su transparencia.
    b.region_rect = Rect2(0, 0, 243, 180)
...
func _kill_boss() -> void:
    if not boss.has("node"):
        return
    var b := boss["node"] as Sprite2D
    var pos := b.position + Vector2(120, 90)
    for i in range(8):
        _boss_explosion(pos + Vector2(rng.randf_range(-85.0, 85.0), rng.randf_range(-55.0, 55.0)))
    b.queue_free()
    boss_active = false
    boss_dead = true
    score += 10000
